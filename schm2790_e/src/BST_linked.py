...
class BST:
    ...
    def _are_adjacents_aux(self, parent, v1, v2):
        ...
        elif (parent._left is not None and parent._right is not None) and \
                (parent._left._value == v1 and parent._right._value == v2 or
                 parent._left._value == v2 and parent._right._value == v1):
            adjacents = True

        else:
            return self._are_adjacents_aux(parent._left, v1, v2) or \
                self._are_adjacents_aux(parent._right, v1, v2)
    ...
    def _max_path_aux(self, node):
        ...
        if node is None:
            ...
            if len(left_path) > len(right_path):
                path = [node._value] + left_path
            else:
                path = [node._value] + right_path
