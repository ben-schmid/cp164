...
class Sorted_List:
        ...
        if index == -1:
            ...
            if previous is None:
                ...
                if previous._next is None:
                    # if none, removed the last value, update none
                    self._rear = previous
    ...
    def remove_many(self, key):
        ...
        if self._count == count:
            ...
            if previous is None:
                # first node removed, update node
                self._front = current
            else:
    ...
    def count(self, key):
        ...
        while current is not None and i != -1:
            current = current._next
            num += 1
    ...
    def clean(self):
        ...
        while compare is not None:
            current = compare._next

            while current is not None and current._value == compare._value:
                compare._next = current._next
                self._count -= 1
                current._next
    ...
    def intersection(self, source1, source2):
        ...
        while source1_node is not None and source2_node is not None:
            ...
            if source1_node._value < source2_node._value:
                # Data does not match, move to next node
                source1_node = source1_node._next
            elif source1_node._value > source2_node._value:
                # Data does not match, move to next mode
                source2_node = source2_node._next
            else:

                if self._front is None:
                    # Add new node to front of self
                    self._front = _SL_Node(source1_node._value, None)
                    self._rear = self._front
                    self._count += 1

                elif self._rear._value < source1_node._value:
                    # Add new node to end of sel
                    self._rear = _SL_Node(source1._node._value, None)
                    self._rear = self._rear._next
                    self._count += 1
                else:
                    # value alread in self, move nodes forwrd
                    source1_node = source1_node._next
                    source2_node = source2_node._next
