...
class BST:
    ...
    def _insert_aux(self, node, value):
        ...
        if node is None:
            # Base Case, add new node containing value
            node = _BST_Node(value)
            self._count += 1
            inserted = True

        elif value < node._value:
            # general case, determining where node needs to go,
            # checking left
            ...
            # base case, value already in BST
            inserted = False
    ...
    def remove_root(self):
        ...
        if self._root._right is None and self._root._right is None:
            self._root = None
            ...
            if self._root._right._left is None:
    ...
    def _eq_aux(self, self_node, target_node):
        ...
        if self_node is None and target_node is None:
            equals = True
                ...
                and self_node._value == target_node._value and \
                self_node._height == target_node._height:
            equals = self._eq_aux(self_node._right, target_node._right) and \
                self._eq_aux(self_node._left, target_node._left)
        else:
            equals = False
        return equals
    ...
    def max_r(self):
        ...
        return self._max_aux(self._root)
    ...
    def _two_child_count_aux(self, node):
        if node is None:
            ...
            count = 1 + self._two_child_count_aux(node._right) + \
                self._two_child_count_aux(node._left)
        else:
    ...
    def _node_counts_aux(self, node):
        if node is None:
            ...
            # base case, root is the only node.
            one = 0
            two = 0
            zero = 1
        elif node._right is None and node._left is not None:
            ...
            zero = zero_r + zero_r
            one = one_r + one_r
            # count the current node as having two children
            two = two_r + two_r + 1
    ...
    def _is_balanced_aux(self, node):
        if node is None or node._height == 1:
            ...
            balanced = self._is_balanced_aux(
                node._right) and self._is_balanced_aux(node._right)
        return balanced
    ...
    def retrieve_r(self, key):
        ...
        -------------------------------------------------------
        """
        value = self._retrieve_aux(self._root, key)
        return value
    ...
    def _retrieve_aux(self, node, key):
        if node is None:
            ...
            if key < node._value:
                ...
                value = self._retrieve_aux(node._right, key)
            else:
                value = deepcopy(node._value)
    ...
    def _is_valid_aux(self, node, min_node, max_node):
        if node is None:
            ...
            balanced = self._is_valid_aux(node._right, None, max_node) and self._is_valid_aux(
                node._left, min_node, None)
        return balanced
    ...
    def inorder(self):
        ...
        """
        a = []
        self._in_order_aux(self._root, a)
        return a

    def _in_order_aux(self, node, a):
        if node is not None:
            self._in_order_aux(node._left, a)
            a.append(node._value)
            self._in_order_aux(node._right, a)
        return
    ...
    def reverse(self):
        ...
        ---------------------------------------------------------
        """
        return self._reverse_aux(self._root)

    def _reverse_aux(self, node):
        if node is not None:
            node._right, node._left = node._left, node._right
