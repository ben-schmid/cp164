...
class BST:
    ...
    def __eq__aux(self, node1, node2):
        if node1 is None and node2 is None:
            ...
            equals = self.__eq__aux(node1._right, node2._right) and self.__eq__aux(
    ...
    def _leaf_count_aux(self, node):
        if node is None:
            ...
            n = self._leaf_count_aux(node._right) + \
                self._leaf_count_aux(node._left)
        return n
    ...
    def remove_root(self):
        ...
        min_node = self._root._right
        while min_node._left is not None:
            min_node = min_node.left

        # Replace the root node with the minimum node
        self._root._value = min_node._value

        # Remove the minimum node from the right subtree
        parent = None
        current = self._root._right
        while current._left is not None:
            parent = current
            current = current._left

        if parent is None:
            self._root._right = current._right
        else:
