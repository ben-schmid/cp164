...
class List:
    ...
    def is_identical_r(self, target):
        ...
        if self._count != target._count:
            ...
            identical = self._is_identical_r_aux(
                source_node, target_node, identical)

        return identical

    def _is_identical_r_aux(self, source_node, target_node, identical):

        if source_node is None:
            identical = True
        elif source_node._value != target_node._value:
            identical = False
        elif source_node._next is None and target_node._next is None:
            identical = True
        else:
            self._is_identical_r_aux(
                source_node._next, target_node._next, identical)
        return identical
    ...
    def union_r(self, source1, source2):
        ...
        source1_node = source1._front
        source2_node = source2._front
        self._union_r_aux(source1_node, source2_node)

    def _union_r_aux(self, source1_node, source2_node):
        if source1_node is not None:
            value = source1_node._value
            _, current, _ = self._linear_search(value)
            ...
            if current is None:
                # Value does not exist in new list.
                self.append(value)
            self._union_r_aux(source1_node._next, source2_node)
        ...
        if source2_node is not None:
            ...
            self._union_r_aux(source1_node, source2_node._next)
